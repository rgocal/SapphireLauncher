...
public class AllTasksListAdapter extends BaseExpandableListAdapter {
    ...
    boolean showDownloaded;
	
	public AllTasksListAdapter(Context context, AllTasksListAsyncProvider.Updater updater) {

        showDownloaded = SettingsProvider.getBoolean(context,
                SettingsProvider.KEY_APPS_SHORTCUT, true);
		...
		for(int i=0; i < all_packages.size(); ++i) {
            ...
            if (  (pack.applicationInfo.flags & (showDownloaded ? ApplicationInfo.FLAG_INSTALLED : ApplicationInfo.FLAG_SYSTEM) ) != 0)
